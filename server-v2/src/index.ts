...
const server = Bun.serve<{ kind: "app" | "browser-native" }>({
  ...
  async fetch(req, server) {
    ...
    if (url.pathname === "/api/browser/status" && req.method === "GET") {
      return jsonResponse(browserBridge.getStatus());
    }
    ...
    if (url.pathname === "/api/browser/command" && req.method === "POST") {
      try {
        ...
        const result = await browserBridge.execute({
    ...
    if (url.pathname === "/api/secrets/command" && req.method === "POST") {
      try {
        ...
        if (body.command === "use") {
          ...
          const result = JSON.parse(resultJson) as { filled?: boolean };
          if (result.filled) await browserBridge.execute({ command: "browser.lock_sensitive_document", payload });
          return jsonResponse({ ok: true, result });
        }
...
async function handleChat(ws: ServerWebSocket, message: ChatRequest) {
  ...
  const requestSkills = learnedSkill ? [learnedSkill, ...(message.skills ?? [])] : message.skills;
  const effectiveMCPServerIds = resolveConversationMCPServerIds(conversation.conversation.id, message.mcpServerIds);
  const agentRequest: ChatRequest = {
    ...
    mcpServers: buildAgentMCPServers(effectiveMCPServerIds),
  };
  let fullText = "";
...
function buildAgentMCPServers(selectedIds?: string[]): MCPServerConfig[] {
  return selectMCPServers(
    [browserMCPServerConfig(), desktopMCPServerConfig(), secretsMCPServerConfig(), ...mcpServers.listEnabled()],
    selectedIds
  );
}

function resolveConversationMCPServerIds(conversationId: string, requestedIds?: string[]) {
...
function browserMCPServerConfig(): MCPServerConfig {
